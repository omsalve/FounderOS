...
import { ChevronDown, ChevronRight, Map, Plus, Check, Loader2, AlertCircle } from "lucide-react";
                ...
                >
                  <motion.div
                    animate={{
                      ...
                      borderColor: checked[i] ? monthColor.bar : "rgba(255,255,255,0.2)",
                      boxShadow: checked[i] ? `0 0 6px ${monthColor.glow}` : "none",
                    }}
                  ...
                  <span
                    ...
                    style={{
                      color: checked[i] ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.6)",
...
function SaveButton({
  ...
  const configs = {
    ...
    saved: {
      ...
      shadow: "0 4px 16px rgba(5,150,105,0.3)",
      cursor: "default",
    },
    error: {
    ...
    <motion.button
      ...
      whileHover={
        saveState === "idle" || saveState === "error"
          ? { y: -3, boxShadow: "0 8px 28px rgba(37,99,235,0.4)" }
          : {}
      }
      whileTap={
        saveState === "idle" || saveState === "error" ? { scale: 0.97 } : {}
      }
      className="w-full rounded-xl py-3 text-sm font-medium text-white uppercase tracking-wide flex items-center justify-center gap-2 transition-all"
      style={{
        background: cfg.bg,
        border: "none",
        cursor: cfg.cursor,
...
export default function AIPlanPanel({ plan, isLoading }: AIPlanPanelProps) {
  ...
  // Reset save state when a new plan comes in
  const [lastSavedGoal, setLastSavedGoal] = useState<string | null>(null);
  const currentSaveState =
    plan && plan.goal !== lastSavedGoal ? saveState : saveState;
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("[Save plan]", res.status, body);
        setSaveState("error");
        return;
      }

      setLastSavedGoal(plan.goal);
      setSaveState("saved");
    } catch (err) {
  ...
  // When the plan changes (new generation), reset save state
  const effectiveSaveState =
    plan?.goal !== lastSavedGoal && saveState === "saved" ? "idle" : saveState;
      ...
      >
        ...
        {plan && (
          ...
          >
            ...
            <span>{plan.months.reduce((a, m) => a + m.weeks.length, 0)} weeks</span>
            <span style={{ color: "rgba(255,255,255,0.15)" }}>·</span>
            <span>
              {plan.months.reduce(
                (a, m) => a + m.weeks.reduce((b, w) => b + w.tasks.length, 0),
                0
              )}{" "}
              tasks
            </span>
        ...
        {isLoading ? (
          <div className="flex h-full flex-col items-center justify-center gap-4">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-white mb-1">Building your plan</p>
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3">
            ...
            <div className="text-center">
              ...
              <p className="text-xs max-w-[200px]" style={{ color: "rgba(255,255,255,0.2)" }}>
          ...
          <div className="space-y-4">
            ...
            <div className="relative ml-4">
              <div
                ...
                style={{
                  background: "linear-gradient(to bottom,rgba(37,99,235,0.4),rgba(124,58,237,0.4),rgba(8,145,178,0.4))",
                }}
              />
            ...
            {/* Save CTA */}
            <SaveButton saveState={effectiveSaveState} onSave={handleSave} />
          </div>
        )}
      </div>
    </div>
